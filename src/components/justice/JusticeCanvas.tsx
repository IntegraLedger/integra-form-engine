/**
 * JusticeCanvas — thin React shell around the canvas justice renderer.
 *
 * Responsibilities:
 *  - ResizeObserver for canvas sizing
 *  - MutationObserver on <html> for dark/light mode (.dark class)
 *  - requestAnimationFrame loop → renderJustice()
 *  - prefers-reduced-motion → static at time=0
 */

import { useEffect, useRef } from 'react';
import { cn } from '@/lib/utils';
import { renderJustice } from './justice-renderer';

interface JusticeCanvasProps {
  className?: string;
}

export function JusticeCanvas({ className }: JusticeCanvasProps) {
