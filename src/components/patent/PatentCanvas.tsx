/**
 * PatentCanvas — thin React shell around the canvas patent renderer.
 *
 * Responsibilities:
 *  - ResizeObserver for canvas sizing
 *  - MutationObserver on <html> for dark/light mode (.dark class)
 *  - requestAnimationFrame loop -> renderPatent()
 *  - prefers-reduced-motion -> static at time=0
 */

import { useEffect, useRef } from 'react';
import { cn } from '@/lib/utils';
import { renderPatent } from './patent-renderer';

interface PatentCanvasProps {
  className?: string;
}

export function PatentCanvas({ className }: PatentCanvasProps) {
